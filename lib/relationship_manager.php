...
class RelationshipManager {
    ...
    private static function isBlockedRelationshipTarget($name): bool {
        $lower = strtolower(trim(strval($name)));
        if ($lower === '') {
            return true;
        }
    ...
    public static function getRelationships($npcName) {
        ...
        $filtered = [];
        foreach ($rels as $target => $data) {
            if (self::isBlockedRelationshipTarget($target)) {
                continue;
            }
    ...
    public static function getRelationship($npcName, $targetName) {
        if (self::isBlockedRelationshipTarget($targetName)) {
            return [
                'aff' => 0,
                'type' => 'neutral',
                'tier' => 'Neutral'
     ...
     * Includes nearby NPC relationships only
     *
    ...
    public static function buildContext($npcName, $nearbyNpcs = []) {
        ...
        foreach ($nearbyNpcs as $target) {
            $target = trim($target);
            if (self::isBlockedRelationshipTarget($target)) continue;
...
EXAMPLES:
Small kindness: "Thank you for the flowers." #REL:Ruka=+5#
Saved your life: "You... you saved me! I won't forget this." #REL:Ruka=+40#
Attacked you: "You dare strike me?!" #REL:Ruka=-35# #TYPE:Ruka=Rival#
Killed your friend: "MURDERER! I will NEVER forgive you!" #REL:Ruka=-70#
PROMPT;
    }
    ...
    public static function buildDirectorContext($npcsInScene = [], $mentionedNpcs = []) {
        ...
        $allNpcs = array_filter($allNpcs, function($n) {
            $n = trim($n);
            return !self::isBlockedRelationshipTarget($n);
        });
        ...
        foreach ($cleanNpcs as $npc) {
            ...
            // Subject/topic affinities (anything that's not an NPC name)
