...
try {
    ...
    if (stobeAutonomyBool($_GET['include_locations'] ?? false)) {
        $response['locations'] = stobeAutonomyListVisitedLocations();
    }
    if (stobeAutonomyBool($_GET['include_pilot'] ?? false)) {
        $response['pilot_steps'] = stobeAutonomyListPilotSteps();
        $response['decisions'] = stobeAutonomyListDecisions();
    }
