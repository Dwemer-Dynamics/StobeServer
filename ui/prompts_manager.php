...
$getDbLastError = static function () use ($db): string {
...
$fetchPrompts = static function () use ($db): array {
    return $db->fetchAll(
        "SELECT prompt_key, default_prompt, custom_prompt, description, created_at, updated_at
         FROM prompts
         ORDER BY prompt_key ASC"
    );
};

$updatePromptCustom = static function (string $promptKey, ?string $customPrompt) use ($db): bool {
    $result = $db->exec(
        "UPDATE prompts
         SET custom_prompt = $1,
             updated_at = CURRENT_TIMESTAMP
         WHERE prompt_key = $2",
        [$customPrompt, $promptKey]
    );
    return $result !== false;
};
...
if (isset($_GET['action']) && $_GET['action'] === 'export_custom') {
    ...
    foreach ($rows as $row) {
        ...
        fputcsv($output, [strval($row['prompt_key'] ?? ''), strval($row['custom_prompt'] ?? '')]);
    }
    fclose($output);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    && strtolower(strval($_SERVER['HTTP_X_REQUESTED_WITH'])) === 'xmlhttprequest'
) {
    if (isset($_POST['action']) && $_POST['action'] === 'update_custom') {
        $prompt_key = trim(strval($_POST['prompt_key'] ?? ''));
        $custom_prompt = $_POST['custom_prompt'] ?? '';

        if ($prompt_key !== '') {
            if (trim(strval($custom_prompt)) === '') {
                $custom_prompt = null;
            } else {
                $custom_prompt = strval($custom_prompt);
            }

            $success = $updatePromptCustom($prompt_key, $custom_prompt);
            $message = $success
                ? ($custom_prompt === null ? 'Custom prompt cleared' : 'Custom prompt updated successfully')
                : ('Error updating prompt: ' . $getDbLastError());
        } else {
            $success = false;
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'clear_custom') {
        $prompt_key = trim(strval($_POST['prompt_key'] ?? ''));
        if ($prompt_key !== '') {
            $success = $updatePromptCustom($prompt_key, null);
            $message = $success ? 'Custom prompt cleared' : ('Error clearing prompt: ' . $getDbLastError());
        } else {
            $success = false;
            $message = 'No prompt key specified';
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'import_custom') {
    if (isset($_FILES['csv_file']) && intval($_FILES['csv_file']['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK) {
        $fileTmpPath = strval($_FILES['csv_file']['tmp_name'] ?? '');
        $fileExtension = strtolower(pathinfo(strval($_FILES['csv_file']['name'] ?? ''), PATHINFO_EXTENSION));
        ...
        if ($fileExtension === 'csv') {
            ...
            if ($handle !== false) {
                ...
                while (($data = fgetcsv($handle, 10000, ',')) !== false) {
                    ...
                    $prompt_key = trim(strval($data[0] ?? ''));
                    $custom_prompt = strval($data[1] ?? '');
                    if ($prompt_key === '') {
                        continue;
                    }

                    $exists = $db->fetchOne(
                        "SELECT prompt_key FROM prompts WHERE prompt_key = $1 LIMIT 1",
                        [$prompt_key]
                    );
                    if (!$exists) {
                        $errorCount++;
                    ...
                    $ok = $updatePromptCustom($prompt_key, $custom_prompt);
                    if ($ok) {
                        $importCount++;
                    } else {
                        $errorCount++;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_custom') {
    ...
    if ($prompt_key !== '') {
        ...
        $ok = $updatePromptCustom($prompt_key, $custom_prompt);
        if ($ok) {
            $message = $custom_prompt === null
                ? "Custom prompt cleared. Using default prompt for <strong>$prompt_key</strong>."
                : "Custom prompt updated successfully for <strong>$prompt_key</strong>.";
        } else {
            $message = 'Error updating prompt: ' . $getDbLastError();
        }
    } else {
        $message = 'No prompt key specified.';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'clear_custom') {
    $prompt_key = trim(strval($_POST['prompt_key'] ?? ''));
    if ($prompt_key !== '') {
        $ok = $updatePromptCustom($prompt_key, null);
        if ($ok) {
            $message = "Custom prompt cleared. Reverted to default prompt for <strong>$prompt_key</strong>.";
        } else {
            $message = 'Error clearing prompt: ' . $getDbLastError();
        }
    }
}

$promptsRows = $fetchPrompts();
$TITLE = "Stobe - Prompts Manager";
$debugPaneLink = false;
...
<style>
    ...
    .warning-box {
        background: linear-gradient(135deg, rgba(230, 183, 108, 0.1), rgba(230, 183, 108, 0.05));
    ...
    .prompts-table thead {
        ...
        border-bottom: 2px solid rgba(230, 183, 108, 0.5);
    ...
    .prompts-table tbody tr:hover {
        ...
        box-shadow: inset 0 0 10px rgba(230, 183, 108, 0.1);
    }
    ...
    .btn-clear {
        background: linear-gradient(135deg, rgba(230, 183, 108, 0.9), rgba(230, 183, 108, 0.9));
        color: white;
        border-color: rgba(230, 183, 108, 0.3);
    }

    .btn-clear:hover {
        background: linear-gradient(135deg, rgba(230, 183, 108, 1), rgba(230, 183, 108, 1));
    ...
    .modal-header {
        padding: 20px;
        background: linear-gradient(180deg, rgba(26, 26, 26, 0.95), rgba(20, 20, 20, 0.98));
        border-bottom: 1px solid rgba(230, 183, 108, 0.3);
    ...
    .modal-footer {
        padding: 15px 20px;
        background: linear-gradient(180deg, rgba(26, 26, 26, 0.95), rgba(20, 20, 20, 0.98));
        border-top: 1px solid rgba(230, 183, 108, 0.3);
        text-align: right;
...
<main>
    ...
    <div class="table-container">
        <table class="prompts-table">
            ...
            <tbody>
                <?php foreach ($promptsRows as $row): 
...
foreach ($promptsRows as $row) {
    ...
    $key = strval($row['prompt_key'] ?? '');
    if ($key === '') {
        continue;
    }
    $promptsDataMap[$key] = [
        'default_prompt' => strval($row['default_prompt'] ?? ''),
        'custom_prompt' => $row['custom_prompt'] ?? null,
        'description' => strval($row['description'] ?? ''),
    ];
}
echo 'promptsData = ' . json_encode($promptsDataMap, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) . ';';
?>
