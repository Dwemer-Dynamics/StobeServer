...
try {
    ...
    if ($method === 'POST') {
        ...
        if (!is_string($_POST['action'] ?? null) || !in_array($_POST['action'], ['switch','new'], true)
    ...
    if ($method === 'GET') {
        ...
        $response = ['ok'=>true] + pth_change($conn, $_POST['action'], array_intersect_key($_POST, array_flip(['profile_id','name','expected_token'])));
