...
function stobe_llm_to_bool_int(mixed $value): int {
    if (is_bool($value)) {
        return $value ? 1 : 0;
    }
...
function stobe_llm_guess_service_from_driver_url(string $driver, string $url): string {
...
function stobe_llm_driver_for_service(string $service, string $currentDriver = ''): string {
    $svc = strtolower(trim($service));
    $cur = strtolower(trim($currentDriver));
    if ($cur !== '') {
        return stobeNormalizeLlmConnectorTypeForStorage($cur);
    }
    return match ($svc) {
...
function stobe_llm_decode_config(mixed $raw): array {
    if (is_array($raw)) {
        return $raw;
    }
...
function stobe_llm_db_row_to_ui_row(array $row): array {
    $config = stobe_llm_decode_config($row['config'] ?? '{}');
    $service = strval($config['service'] ?? '');
    if ($service === '') {
        $service = stobe_llm_guess_service_from_driver_url(
            strval($row['connector_type'] ?? ''),
            strval($row['base_url'] ?? '')
        );
    }
    ...
    $ui['frequency_penalty'] = $config['frequency_penalty'] ?? '';
    $ui['repetition_penalty'] = $config['repetition_penalty'] ?? '';
    $ui['top_p'] = $config['top_p'] ?? '';
    $ui['top_k'] = $config['top_k'] ?? '';
    $ui['min_p'] = $config['min_p'] ?? '';
    $ui['top_a'] = $config['top_a'] ?? '';
    $ui['enforce_json'] = stobe_llm_to_bool_int($config['enforce_json'] ?? 0);
...
function stobe_llm_compose_config_from_payload(array $payload, array $existingConfig = []): array {
    $cfg = $existingConfig;
    $cfg['service'] = strtolower(trim(strval($payload['service'] ?? ($cfg['service'] ?? ''))));
    $cfg['provider'] = trim(strval($payload['provider'] ?? ($cfg['provider'] ?? '')));

    foreach (['presence_penalty', 'frequency_penalty', 'repetition_penalty', 'top_p', 'min_p', 'top_a'] as $f) {
        if (array_key_exists($f, $payload) && trim(strval($payload[$f])) !== '') {
            $cfg[$f] = floatval($payload[$f]);
        } elseif (array_key_exists($f . '_is_null', $payload) && stobe_llm_to_bool_int($payload[$f . '_is_null']) === 1) {
            unset($cfg[$f]);
        }
    }
    if (array_key_exists('top_k', $payload) && trim(strval($payload['top_k'])) !== '') {
        $cfg['top_k'] = intval($payload['top_k']);
    } elseif (array_key_exists('top_k_is_null', $payload) && stobe_llm_to_bool_int($payload['top_k_is_null']) === 1) {
        unset($cfg['top_k']);
    }
    ...
    $metadata = [];
    if (isset($payload['metadata']) && trim(strval($payload['metadata'])) !== '') {
        $decodedMeta = json_decode(strval($payload['metadata']), true);
        if (is_array($decodedMeta)) {
            $metadata = $decodedMeta;
        }
    } elseif (isset($cfg['metadata']) && is_array($cfg['metadata'])) {
...
function stobe_llm_payload_to_save_fields(array $payload, ?array $existingDbRow = null): array {
    $existingConfig = $existingDbRow ? stobe_llm_decode_config($existingDbRow['config'] ?? '{}') : [];
    $driver = stobe_llm_driver_for_service(
        strval($payload['service'] ?? ''),
        strval($payload['driver'] ?? ($existingDbRow['connector_type'] ?? ''))
    );
    $apiBadgeId = null;
    if (array_key_exists('api_badge_id', $payload) && trim(strval($payload['api_badge_id'])) !== '') {
        $apiBadgeId = intval($payload['api_badge_id']);
    } elseif ($existingDbRow && array_key_exists('api_badge_id', $existingDbRow)) {
        $apiBadgeId = $existingDbRow['api_badge_id'] === null ? null : intval($existingDbRow['api_badge_id']);
    }

    $fields = [
        'name' => trim(strval($payload['label'] ?? ($existingDbRow['name'] ?? ''))),
    ...
    if ($existingDbRow && isset($existingDbRow['id'])) {
        $fields['id'] = intval($existingDbRow['id']);
    }
...
function stobe_llm_generate_clone_label(string $baseName): string {
    $rows = getAllLlmConnectors();
    $nameSet = [];
    foreach ($rows as $r) {
        $nameSet[strtolower(strval($r['name'] ?? ''))] = true;
    }
    $candidate = $baseName . ' Copy';
    if (!isset($nameSet[strtolower($candidate)])) {
        return $candidate;
    }
    $i = 2;
    while ($i < 5000) {
    ...
    return $baseName . ' Copy ' . time();
}

function stobe_llm_default_api_badge_id(): ?int {
    $db = $GLOBALS["db"] ?? null;
    if (!$db) {
        return null;
    }
    $row = $db->fetchOne(
        "SELECT id
         FROM core_api_badge
         ORDER BY CASE WHEN lower(label) = 'openrouter' THEN 0 ELSE 1 END, id ASC
         LIMIT 1"
    );
    $id = intval($row['id'] ?? 0);
    return $id > 0 ? $id : null;
...
class LLMConnector {
    public function readAll(): array {
        ...
        $mapped = [];
        foreach ($rows as $row) {
            $mapped[] = stobe_llm_db_row_to_ui_row($row);
        }
        return $mapped;
    }
    ...
    public function getById(mixed $id): array|false {
        ...
        return stobe_llm_db_row_to_ui_row($row);
    }

    public function create(array $payload): int {
        $fields = stobe_llm_payload_to_save_fields($payload, null);
    ...
    public function update(mixed $id, array $payload): int {
        ...
        $fields = stobe_llm_payload_to_save_fields($payload, $existing);
        return saveLlmConnector($fields);
    }
    ...
    public function clone(mixed $id): int {
        ...
        $clone = $existing;
        unset($clone['id']);
        $clone['name'] = stobe_llm_generate_clone_label(strval($existing['name'] ?? 'Connector'));
        $clone['is_default'] = false;
        return saveLlmConnector($clone);
    }
}
...
// Determine web root and bring in site chrome like world_knowledge_upload
...
$withEmbed = function (string $url) use ($isEmbed): string {
    ...
    return $url . (strpos($url, '?') === false ? '?embed=1' : '&embed=1');
};
...
.llm-left .llm-title { font-family: 'Exo2', Arial, sans-serif; word-spacing: normal; }
...
/* Fallback: force Herika-style connector buttons even if external CSS is cached/missing */
.btn-base,
.btn-danger,
.btn-cancel,
.btn-save,
...
.btn-danger { background-color: rgba(166, 53, 63, 0.9); border-color: rgba(255, 99, 99, 0.3); }
.btn-save { background-color: #176529; border-color: rgba(72, 187, 120, 0.3); }
.btn-primary {
    background-color: rgb(1 53 166 / 90%);
...
if (isset($_GET["partial"]) && $_GET["partial"] === "editor") {
    ...
    .service-icon.active { outline:2px solid #e6b76c; opacity:1; }
    ...
    if (isInIframe) {
        window.handleEmbeddedSave = async function() {
            ...
            try {
                ...
                const res = await fetch('llm_connectors.php', { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest' }, body: fd });
                let json = {};
    ...
    if (testBtn){
        testBtn.addEventListener('click', async function(){
            const form = testBtn.closest('form') || document.getElementById('llm_editor_form') || document.querySelector('form[method="post"]');
            if (!form) return;
            try {
            ...
            openModal('<?= $webRoot; ?>/ui/llmtest.php' + (cid ? ('?connector_id='+encodeURIComponent(cid)) : ''));
        });
        }
    })();
    // API key notice
    ...
    (function(){
        ...
        function renderList(models, filterText){
            ...
            else { list.forEach(m => { const prompt = formatPrice(m.pricing && m.pricing.prompt); const completion = formatPrice(m.pricing && m.pricing.completion); const ctxRaw = (m.top_provider && m.top_provider.context_length) || m.context_length || ''; const ctx = formatContext(ctxRaw); const name = m.name ? ' ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ' + escapeHtml(m.name) : ''; const line = `${escapeHtml(m.id)}${name}`; const sub = `Pricing (per 1M tokens): input ${prompt} ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ output ${completion}` + (ctx? ` ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ context ${ctx}` : ''); html += `<div class=\"orm-item\" data-id=\"${encodeHtmlAttr(m.id)}\" title=\"${encodeHtmlAttr(m.description||m.name||m.id)}\"><div>${line}</div><div class=\"orm-muted\" style=\"font-size:12px; margin-top:2px;\">${sub}</div></div>`; }); }
        ...
        (function(){
            ...
            function renderInfo(m){ const prompt = formatPrice(m.pricing && m.pricing.prompt); const completion = formatPrice(m.pricing && m.pricing.completion); const ctx = formatContext(ctxOf(m)); return `<div style=\"font-weight:600; margin-bottom:4px;\">OpenRouter model info</div><div class=\"orm-muted\" style=\"font-size:12px;\">Pricing (per 1M tokens): Input ${prompt} ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ Output ${completion}${ctx? ` ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ Context ${ctx}`: ''}</div>`; }
    ...
    (function(){
        ...
        function renderGroqList(models, filterText){
            ...
            if (list.length === 0){ html += '<div class="orm-muted" style="padding:8px 10px;">No matches</div>'; }
            else { list.forEach(m => { const ctx = m.context_window ? formatContext(m.context_window) : ''; const owner = m.owned_by || 'Groq'; const sub = owner + (ctx ? ` ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ context ${ctx}` : ''); html += `<div class=\"orm-item\" data-id=\"${encodeHtmlAttr(m.id)}\" title=\"${encodeHtmlAttr(m.id)}\"><div>${escapeHtml(m.id)}</div><div class=\"orm-muted\" style=\"font-size:12px; margin-top:2px;\">${sub}</div></div>`; }); }
        ...
        async function loadGroqModels(){
            ...
            ensureGroqDropdown();
            groqDropdown.innerHTML = '<div class="orm-head">Groq Models</div><div class="orm-note">LoadingÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦</div>';
            positionGroqDropdown();
            try {
...
if (isset($_GET["delete"])) {
        ...
        $cntRow = $GLOBALS["db"]->fetchOne(
            "SELECT COUNT(*) AS cnt
             ...
             WHERE llm_primary_id = $1
                ...
                OR backgroundlife_connector = $1
                OR dynamic_connector = $1
                OR relationship_connector = $1",
            [$id]
        );
    ...
    if ($inUse > 0) {
        ...
        header("Location: " . $withEmbed("llm_connectors.php?notice=" . urlencode($msg)));
        exit;
    }
    $llm->delete($id);
    header("Location: " . $withEmbed("llm_connectors.php"));
    exit;
}
...
<div class="page-header stobe-page-head">
    <h1 class="api-title stobe-page-head-title">LLM Connectors</h1>
    <p class="page-subtitle stobe-page-head-note">Configure Language Model connectors for AI dialogue generation</p>
</div>
...
<div class="llm-layout">
    <div class="llm-left position-sticky">
        ...
        (function(){
            ...
            const LIST_ACTION = <?= json_encode($withEmbed('llm_connectors.php'), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE) ?>;
            const EMBED_HIDDEN = <?= json_encode($isEmbed ? '<input type="hidden" name="embed" value="1">' : '', JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE) ?>;
            const EDIT_SUFFIX = <?= json_encode($isEmbed ? '&embed=1' : '', JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE) ?>;
            ...
            function render(){
                ...
                rows.forEach(r=>{
                    ...
                    html += `
                        <div class="conn-li${active}" data-id="${String(r.id)}">
                            ...
                            <div class="actions">
                                <form method="get" action="${LIST_ACTION}" onsubmit="return confirm('Are you sure you want to delete this connector?');" style="display:inline">
                                    <input type="hidden" name="delete" value="${String(r.id)}">
                                    ${EMBED_HIDDEN}
                                    <button type="submit" class="btn-danger">Delete</button>
                                </form>
                                <form method="get" action="${LIST_ACTION}" style="display:inline">
                ...
                list.querySelectorAll('.conn-li').forEach(li => {
                    li.addEventListener('click', (ev) => {
                        ...
                        if (id) window.location.href = `?edit=${id}${EDIT_SUFFIX}`;
                    });
                });
            }
            render();
...
.conn-li.active { 
    ...
    box-shadow: 0 4px 12px rgba(230, 183, 108, 0.3);
}
.conn-li .head { display:flex; justify-content:space-between; gap:8px; align-items:center; }
.conn-li .title { font-weight:600; color:#e9efff; font-family:'Exo2', Arial, sans-serif !important; word-spacing: normal; }
...
<form id="llm_editor_form" method="post" onsubmit='return consolidation()' style='<?= $editItem!=null?"":"display:none"?>'>
    <?php if ($editItem): ?>
        <input type="hidden" name="id" value="<?= $editItem["id"] ?>">
    <?php endif; ?>
        ...
        <div>
            <div class="top-actions" style="display:flex; gap:8px; align-items:center; margin-bottom:8px;">
                <?php if ($editItem): ?>
                    ...
                    <button type="submit" formmethod="get" formaction="<?= htmlspecialchars($withEmbed('llm_connectors.php')) ?>" name="export" value="<?= htmlspecialchars($editItem['id'] ?? '') ?>" class="btn-primary">Export</button>
