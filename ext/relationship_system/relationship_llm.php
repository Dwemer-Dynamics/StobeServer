...
class RelationshipLLM {
    ...
    private $modelName;
    private $promptCache = [];
    private $promptsTableAvailable = null;
    ...
    private function loadPrompt($promptKey, $fallback) {
        ...
        try {
            if ($this->db) {
                if ($this->promptsTableAvailable === null) {
                    $exists = $this->db->fetchOne("SELECT to_regclass('public.prompts') AS rel");
                    $this->promptsTableAvailable = is_array($exists) && !empty($exists['rel']);
                }
                if ($this->promptsTableAvailable) {
    ...
    private function initConnector() {
        ...
        // Stobe default: use default profile relationship connector, then response connector.
        if ($connectorId <= 0 && $this->db) {
            $profileRow = $this->db->fetchOne(
            ...
            if (is_array($profileRow)) {
                $relId = intval($profileRow['relationship_connector'] ?? 0);
                $respId = intval($profileRow['response_connector'] ?? 0);
                if ($relId > 0) {
                    $connectorId = $relId;
                } elseif ($respId > 0) {
                    $connectorId = $respId;
        ...
        // Fallback to best available connector:
        // 1) default with API key, 2) default, 3) any with API key, 4) first.
        if (empty($this->connector)) {
            ...
            if (!empty($connectors)) {
                ...
                foreach ($connectors as $row) {
                    ...
                    $hasKey = trim(strval($row['api_key'] ?? '')) !== '';
                    if ($isDefault && $hasKey && !$defaultWithKey) {
                        $defaultWithKey = $row;
                    }
                    if ($isDefault && !$defaultAny) {
                        $defaultAny = $row;
                    }
                    if ($hasKey && !$withKey) {
                        $withKey = $row;
                ...
                $this->connector = $defaultWithKey ?: ($defaultAny ?: ($withKey ?: $connectors[0]));
            }
        }
...
PROMPT;
        ...
        return $this->loadPrompt('rel_llm_analysis', $fallback);
    }
    ...
    private function parseResponse($response) {
        ...
        foreach ($parsed['relationships'] as $target => $data) {
            ...
            if ($this->isBlockedRelationshipTarget($target)) {
                continue;
            }
    ...
    public function inferTransitiveRelationships($npcId) {
        ...
        foreach ($myRels as $targetName => $targetData) {
            ...
            $targetNpc = $this->db->fetchOne(
                "SELECT id, extended_data FROM core_npc_master WHERE name = '" . $escapedTarget . "' LIMIT 1"
            );
     ...
     * @param array $context Full context including recent events and dialogue
     * @return array Changes to apply: ['TargetNpc' => ['delta' => +15, 'type' => null], ...]
     */
    public function evaluateContext($npcId, $npcResponse, $context = []) {
        ...
        $listenerName = trim(strval($context['listener_name'] ?? ''));
        if ($listenerName === '' || $this->isBlockedRelationshipTarget($listenerName)) {
            return ['ok' => true, 'changes' => [], 'reason' => 'Listener missing or blocked'];
        }
        ...
        if (!empty($context['director_instruction'])) {
            $contextStr .= "DIRECTOR INSTRUCTION:\n";
            $contextStr .= "\"" . $context['director_instruction'] . "\"\n";
            $contextStr .= "Treat this as guidance context.\n\n";
        }
        if (!empty($context['events'])) {
            $contextStr .= "Recent Events:\n" . implode("\n", array_slice($context['events'], -10)) . "\n\n";
        }

        $listenerKey = $listenerName;
        $contextStr .= "CONVERSATION:\n";
        if (!empty($context['listener_utterance'])) {
            $contextStr .= "[{$listenerKey} said]: " . $context['listener_utterance'] . "\n";
        }
        ...
        $relStateStr = "";
        $listenerRel = $currentRels[$listenerKey] ?? ['aff' => 0, 'type' => 'neutral'];
        $relStateStr .= "  {$listenerKey}: {$listenerRel['aff']} ({$listenerRel['type']})\n";

        $nearbyNpcs = $context['nearby_npcs'] ?? [];
        $mentionedNpcs = [];
        $textToScan = $npcResponse;
        if (!empty($context['dialogue'])) {
        ...
        foreach (array_keys($currentRels) as $knownNpc) {
            if ($this->isBlockedRelationshipTarget($knownNpc)) {
                continue;
            }
        ...
        foreach ($relevantNpcs as $npcTarget) {
            ...
            if ($npcTarget === '' || $this->isBlockedRelationshipTarget($npcTarget) || $npcTarget === $listenerKey) {
                continue;
            }
...
PROMPT;
    ...
    private function applyChanges($npcId, $changes, $currentRels) {
        ...
        foreach ($changes as $target => $change) {
            ...
            if ($this->isBlockedRelationshipTarget($target)) {
                Logger::debug("[REL-LLM] Skipping blocked non-NPC target: {$target}");
                continue;
            }
    ...
    private function isBlockedRelationshipTarget($target): bool {
        $lower = strtolower(trim(strval($target)));
        if ($lower === '') {
            return true;
        }
