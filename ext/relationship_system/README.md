...
Set `RELLLM_CONNECTOR` in Global Settings UI or your runtime bootstrap config:
```php
...
[RELATIONSHIPS]
Kang: Fond (Platonic)
Ruka: Attached (Professional)
```

### Mode 2: Command-Based (Fallback)

If `RELLLM_CONNECTOR` is not set:
- Conversation model handles scoring via `#REL:Kang=+15#` commands
- More token overhead but doesn't require separate LLM

**What conversation model sees:**
```
[RELATIONSHIPS]
Kang: +35 (Fond, Platonic)
Ruka: +55 (Attached, Professional)
```
...
Or add in your bootstrap/runtime config:
```php
...
// Get relationship
$rel = RelationshipManager::getRelationship('Lydia', 'Kang');
// Returns: ['aff' => 45, 'type' => 'romantic', 'tier' => 'Fond']

// Set relationship
RelationshipManager::setRelationship('Lydia', 'Kang', 80, 'romantic');
```
