 ...
 * - Relationship updates are NPC-to-NPC only
 *
 * ASYNC FLOW:
 ...
 *    - Conversation model embeds #REL:TargetNpc=+5# in responses
 *    - Traditional approach (synchronous but fast - no LLM call)
 *
 * NPC-TO-NPC RELATIONSHIPS:
 * When one NPC talks to another NPC, both NPCs form/update
 ...
 * Returns false for Narrator, empty, or legacy player placeholders
 */
...
if ($isNpcToNpcConversation) {
    ...
    Logger::debug("[REL] Skipping relationship queue for non-NPC listener: " . ($listenerName ?? 'NULL'));
    return;
}
...
if ($useRelLLM && $npcId) {
    ...
    $context['listener_name'] = $listenerName;
    ...
    Logger::info("[REL] Queueing {$npcName}: request_type={$requestType}, is_npc2npc=" . ($isNpcToNpcConversation ? 'YES' : 'NO'));
