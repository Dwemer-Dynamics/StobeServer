 ...
 *   - context: Additional context (events, instruction hints, etc)
        ...
        foreach ($rows as $row) {
            ...
            try {
                ...
                if ($isNpcToNpc) {
                    ...
                    Logger::debug("[REL-ASYNC] Skipping non-NPC relationship eval for {$data['npc_name']}");
                }
...
function _relProcessInitQueue($limit = 5) {
    ...
    try {
        ...
        foreach ($rows as $row) {
            ...
            try {
                // Build initial JSONB relationships from event history
                $initResult = $relLLM->analyzeNpc($data['npc_id'], false);
                if (!empty($initResult['ok']) && empty($initResult['skipped'])) {
